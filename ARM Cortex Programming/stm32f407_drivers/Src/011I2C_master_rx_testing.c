 ...
 *      Author: Jason
 *
 *      test I2C by receiving some bytes from an Arduino slave
 *
 *      for I2C1 we use
 *      I2C1 SCL => PB6
 *      I2C1 SDA => PB7
...
int main(void)
{
	...
	while(1) //loop button presses
	{
		...
		delay(500000*2);
			...
			I2C_MasterSendData(&I2C1Handle, &pTxBuffer, 1, SLAVE_ADDR, I2C_ENABLE_SR);  //send data

			//now receive the length
			I2C_MasterReceiveData(&I2C1Handle, &Len, 1, SLAVE_ADDR, I2C_ENABLE_SR);  //receive data
			...
			//receive data
			I2C_MasterReceiveData(&I2C1Handle, pRxBuffer, Len, SLAVE_ADDR, I2C_DISABLE_SR);  //receive data
