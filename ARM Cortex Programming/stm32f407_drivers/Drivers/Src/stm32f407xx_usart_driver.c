...
void USART_SetBaudRate(USART_RegDef_t *pUSARTx, uint32_t BaudRate)
{
  ...
  if(pUSARTx == USART1 || pUSARTx == USART6)
  {
	   ...
	   PCLKx = RCC_GetAPB2PCLKValue();
  }else
  {
	   PCLKx = RCC_GetAPB1PCLKValue();
  }
...
//StatusFlagName must be one of the #define values for the SR register
// e.g. USART_SR_RXNE
...
/*************************Check for TC flag ********************************************/
	...
	if(temp1 && temp2 )
	{
		...
		//Jason note. i still think we should check for TxLen > 0, and if so, we should write data to DR.
		//we can't assume the TC flag will be set ONLY if sending is complete (TxLen == 0). There could be other reasons
